...
fn is_python_cache_path(relative: &str) -> bool {
    is_python_bytecode_file(relative) || is_python_cache_directory_path(relative)
}

fn is_python_cache_directory_path(relative: &str) -> bool {
    relative
        .split('/')
...
fn clean_python_cache(
    trusted_base: &Path,
    root: &Path,
    label: &str,
    remove_legacy_bytecode: bool,
) -> Result<PluginTreeSnapshot, String> {
    let snapshot = scan_plugin_tree(trusted_base, root, label)?;
    for relative in snapshot.files.iter().filter(|relative| {
        is_python_bytecode_file(relative)
            && (remove_legacy_bytecode || is_python_cache_directory_path(relative))
    }) {
...
fn clean_installed_python_cache(
    ...
    clean_python_cache(trusted_base, root, label, true)
}

fn clean_development_bundled_python_cache(
    trusted_base: &Path,
    root: &Path,
    label: &str,
) -> Result<PluginTreeSnapshot, String> {
    clean_python_cache(trusted_base, root, label, false)
}
...
pub fn verify_bundled_plugin(
    ...
    // Development runs execute plugin scripts directly from target/debug. Python
    // may leave bytecode caches there despite the task runtime disabling writes.
    // Remove only verified cache artifacts before checking the signed tree; release
    // builds stay fail-closed for every unexpected file.
    if cfg!(debug_assertions) {
        clean_development_bundled_python_cache(bundled_plugins, &lexical_root, "内置插件开发缓存")?;
    }
...
mod tests {
    ...
    fn bundled_hash_catalog_covers_all_plugins_and_detects_tampering() {
        ...
        {
            let bytecode = release_plugins
                .join("dingtalk")
                .join("backend")
                .join("__pycache__")
                .join("export_dingtalk.cpython-311.pyc");
            fs::create_dir_all(bytecode.parent().unwrap()).unwrap();
            fs::write(&bytecode, b"generated bytecode").unwrap();
            verify_bundled_plugin(&release_plugins, "dingtalk").unwrap();
            assert!(!bytecode.exists());

            let misplaced_bytecode = release_plugins
                .join("dingtalk")
                .join("backend")
                .join("unsigned.PYC");
            fs::write(&misplaced_bytecode, b"unsigned bytecode").unwrap();
            let error = verify_bundled_plugin(&release_plugins, "dingtalk").unwrap_err();
            assert!(
                error.contains("额外文件") && error.contains("unsigned.PYC"),
                "{error}"
            );
            fs::remove_file(misplaced_bytecode).unwrap();

            let cache_payload = release_plugins
                .join("dingtalk")
                .join("backend")
                .join("__pycache__")
                .join("payload.txt");
            fs::create_dir_all(cache_payload.parent().unwrap()).unwrap();
            fs::write(&cache_payload, b"unsigned payload").unwrap();
            let error = verify_bundled_plugin(&release_plugins, "dingtalk").unwrap_err();
            assert!(
                error.contains("额外文件") && error.contains("payload.txt"),
                "{error}"
            );
            fs::remove_file(cache_payload).unwrap();
            fs::remove_dir_all(
                release_plugins
                    .join("dingtalk")
                    .join("backend")
