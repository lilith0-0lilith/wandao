(function (root) {
  ...
  const RULES = [
    { code: 'RESOURCE_DOWNLOAD_FAILED', category: 'resource', categoryLabel: '图片或附件下载失败', userMessage: '图片或附件处理失败', recovery: '正文可能已完成；可以在任务中心单独重试失败资源。', retryable: true, pattern: /图片下载失败|附件下载失败|download.*(?:image|attachment)|(?:图片|附件|资源).{0,40}(?:HTTP\s+40[134]|响应|下载失败|上传失败|处理失败|失败|错误)|(?:image|attachment|resource).{0,40}(?:HTTP\s+40[134]|response|download|upload|fail|error)|tcs-devops\.aliyuncs\.com|cdn\.nlark\.com|imageFailure|imageFailures|上传附件失败/i },
